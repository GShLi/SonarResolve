        ...
        try:
            ...
            if recent_projects:
                ...
                for project in recent_projects[:5]:  # 只显示前5个项目
                    logger.info(f"    - {project['sonar_project_key']} -> {project['jira_project_key']} ({project['created_time']})")
...
"""
        ...
        if successful_projects:
            ...
            for project_key in successful_projects:
                result = results['project_results'][project_key]
                print(f"  📋 {project_key} → {result['jira_project_key']}: {result['jira_tasks_created']}/{result['sonar_issues_count']} 个任务")
